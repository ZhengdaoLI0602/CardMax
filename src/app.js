...
function hkdEquivalent(tx) {
  // v0.1 intentionally avoids inventing FX rates. CNY is treated as 1:1 promo units
  // for the current user-confirmed CN reward-cap calculations; HKD is exact.
  if (tx.currency === 'HKD' || tx.currency === 'CNY') return tx.amount;
  return null;
}

function parseRows(rows) {
  const ledgerWanted = normalizeText(state.rules.profile.ledger).toLowerCase();
  const parsed = rows.map((row, index) => {
    ...
    tx.signedAmount < 0 &&
    tx.ledger.toLowerCase() === ledgerWanted
  );
...
function evaluate(transactions) {
  ...
  const evaluated = transactions.map(tx => {
    ...
    if (tx.card.id === 'aeon_purple') {
      ...
      if (tx.region === 'HK' && (isDining(tx) || ['MTR'].includes(tx.merchant))) rule = r.aeon_purple.HK.localSelected;
      if (rule) {
        grossRate = rule.grossRate;
...
function renderSummary(result) {
  ...
  if (region === 'CN') {
    ...
    cards.push(progressCard({
      ...
      used: result.hsbcCNSpend, cap: target, remainingSpend: null,
  ...
  if (region === 'HK') {
    ...
    cards.push(progressCard({ title: 'HSBC Red · 网购', rate: '4%', used: Math.min(p.redOnlineSpend, red.monthlySpendCapHKD), cap: red.monthlySpendCapHKD, remainingSpend: red.monthlySpendCapHKD - p.redOnlineSpend, advice: 'HKD 网购不收 1.95% 外币手续费。' }));
    const chill = r.bochk_chill.online;
    cards.push(progressCard({ title: 'Chill World · 网购', rate: '5%', used: p.chillExtraReward, cap: chill.monthlyExtraRewardCapHKD, remainingSpend: (chill.monthlyExtraRewardCapHKD - p.chillExtraReward) / chill.extraRate, advice: '额外 4.6% 现金回赠按共享奖励池计算。' }));
  }

  if (region === 'OVERSEAS') {
    cards.push(progressCard({ title: 'AEON Purple · 海外', rate: '≈5% net', advice: '当前登记活动按 6% 积分等值 − 1% 手续费估算。' }));
    const chill = r.bochk_chill.overseas;
    cards.push(progressCard({ title: 'Chill World · 海外/网上', rate: '≈3.05% net', used: p.chillExtraReward, cap: chill.monthlyExtraRewardCapHKD, remainingSpend: (chill.monthlyExtraRewardCapHKD - p.chillExtraReward) / chill.extraRate, advice: '以非 HKD 签账时扣除约 1.95% 手续费。' }));
    const red = r.hsbc_red.online;
    cards.push(progressCard({ title: 'HSBC Red · 海外网购', rate: '≈2.05% net', used: Math.min(p.redOnlineSpend, red.monthlySpendCapHKD), cap: red.monthlySpendCapHKD, remainingSpend: red.monthlySpendCapHKD - p.redOnlineSpend, advice: '日本 SUICA / FamilyMart 等指定商户应优先使用独立 8% 规则。' }));
  ...
  panel.innerHTML = `<div class="section-heading"><div><p class="eyebrow">${region}</p><h2>${state.monthKey} 消费建议</h2></div></div><div class="reward-list">${cards.join('')}</div>`;
}

function renderTransactions(evaluated) {
  const body = $('#transactionTableBody');
  if (!evaluated.length) {
    body.innerHTML = '<tr><td colspan="7" class="muted">本月没有符合条件的信用卡消费。</td></tr>';
    return;
  }
...
async function handleWorkbook(file) {
  ...
  $('#dataUpdated').textContent = `${state.monthKey} · ${file.name}`;
